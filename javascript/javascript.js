...
function createTable() {
  ...
  // Here we will check to make sure all values are numeric.
  if(row_start_string !== String(~~Number(row_start_string)) ||
      row_end_string !== String(~~Number(row_end_string)) ||
      column_start_string !== String(~~Number(column_start_string)) ||
      column_end_string !== String(~~Number(column_end_string))) {
    writeError('Invalid data types entered!  Please enter numbers.');
    return;
  }
  ...
  if(column_end < column_start || row_end < row_start) {
    writeError('Invalid ranges entered!');
    return;
  }
